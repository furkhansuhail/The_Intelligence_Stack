...
The true objective in supervised learning is to minimize the expected loss over the unknown data distribution:

        Expected Risk = E[L(f(x; θ), y)]

But because we do not know the true distribution, we approximate this using the average loss over the training dataset:

        Empirical Risk = (1/N) Σ L(f(x_i; θ), y_i)

Training is therefore the process of minimizing empirical risk in the hope that it approximates expected risk.
